...
panel.grid.minor  = element_line(colour = "#1C2D52", size = I(.1), linetype = "dotted",lineend = "butt", inherit.blank = FALSE), 
      panel.spacing = margin(rep(1,4),unit="pt"), 
      ...
      strip.text.y = element_text(size = 1, color = "white",angle = -90),  
...
Actcolors <- c("Meditation"="#6839B0","Productivity"="#357AA2","Distraction"="#FF7549","RStudio"="#75AADB")
...
# fix(RTCatActs)
ggplot(data = RTCatActs,mapping=aes(y=sum))+
geom_bar(stat="identity",mapping=aes(x=reorder(Activity,sum),fill=Productivity))+
facet_wrap(~Overview,scale="free")+
  theme(legend.key.size = unit(2,"mm"),
        axis.text.x=element_text(vjust=1,hjust=1),
        axis.text.y = element_text(vjust=.7,hjust=.9))+
  coord_flip()
```
